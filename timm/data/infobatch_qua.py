...
class InfoBatch(Dataset):
    ...
    def prune(self):
        ...
        ind = self.scores<self.scores.mean()
        well_learned_samples = np.where(ind)[0]
        b = len(well_learned_samples)
        pruned_samples = []
        higher=np.where(np.invert(b))[0]
        selected = np.random.choice(well_learned_samples, int(self.ratio*len(well_learned_samples)),replace=False)
        a = len(higher)
        br = len(selected)
        #ratio_a = ()
        ratio_a = (a-b+math.sqrt((b-a)**2+4*a*br))/2./a if a!=0 else 0
        if a*ratio_a>=1:
            pruned_samples.extend(np.random.choice(higher, int(a*ratio_a),replace=False))
